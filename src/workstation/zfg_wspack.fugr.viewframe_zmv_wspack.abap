*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZMV_WSPACK
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZMV_WSPACK          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZMV_WSPACK'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZMV_WSPACK_EXTRACT
TOTAL = ZMV_WSPACK_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZMV_WSPACK'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZMV_WSPACK-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZMV_WSPACK_EXTRACT
TOTAL = ZMV_WSPACK_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZMV_WSPACK-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZMV_WSPACK'
...
TOTAL = ZMV_WSPACK_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZMV_WSPACK-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZMV_WSPACK-UPD_FLAG NE SPACE.
